...
public class Engine implements Definitions {
	...
	private int totalNodesSearched; // The total number of nodes for the entire search
	private long startTime; // The time we started searching at
	private boolean stopSearch; // Used for hard stops
	private int timeForThisMove; // Set if we are using a fixed time for every move
	private Move[][] searchMoves; // This will be filled with moves to search
	private int current_depth; // The depth we are currently searching to in the iterative deepening
	private int nextTimeCheck; // Keeps track of when to check the time
	//private int globalBestMove; // Keeps track of the best move (to not rely fully on the tt for this)
	private boolean useFixedDepth; // Is this a fixed depth search or not?
	private KillerMoves killers; // Keeps track of killer moves
	private LineEval finalEval;
	private int[][] historyValues;
	private int[][] historyBetaHits;
	private int rootMovesCount;
	private boolean ponder;
	private Uci uci = null;
	private Evaluation evaluation = null;
	private See see = null;
	
	public Engine(Uci uci) {
		this.uci = uci;
		evaluation = new Evaluation();
		see = new See();
	}
	
	public LineEval search(Board board, int depth, int timeLeft, int increment, int movetime, boolean isPonder) throws IOException {
		// Some initalizations
		...
		for(current_depth = 1; current_depth <= 64;) {
			...
			receiveThinking(startTime, finalEval);
//			System.out.println(receiveThinking(startTime, finalEval)); // Get a thinking string and send
	...
	public Move alphaBetaRoot(Board board, int depth, int alpha, int beta, boolean allowNull, int ply) throws IOException {
...
//			}
	...
	public int alphaBeta(Board board, int depth, int alpha, int beta, boolean allowNull, int ply) throws IOException {
	 ...
	 * @return
	 */
	private int quiescentSearch(Board board, int alpha, int beta, int ply) {
	  ...
	  * @return
	  */
	private int calculateTime(Board board, int timeLeft, int increment) {
	...
	public boolean shouldWeStop() throws IOException {
		...
		String line = uci.getCurrentCommand();
		if("stop".equals(line))	return true;
		if("ponderhit".equals(line)) ponder = false;
		return false;
	} // END shouldWeStop
	 ...
	 */
	public synchronized static boolean isInCheck(Board board) {
		int king_square;		

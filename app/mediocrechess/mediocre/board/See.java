...
public class See implements Definitions {
	private int move_to; // Holds the square where the captures take
								// place
	private int move_from; // Holds the square from the initial
									// attacker
	private int b_attackers_total; // Keeps track of the number of
											// white attackers
	private int w_attackers_total; // Keeps track of the number of
											// black attackers
	private Board board; // A pointer to the inputBoard so we don't have
								// to pass it around
	...
	private final int[] piece_values = { 0, 1, 3, 3, 5, 9, 99, 0, 99, 9,
			5, 3, 3, 1 };
	private int[] w_attackers = new int[16];
	private int[] b_attackers = new int[16];
	private int[] scores = new int[32]; // Holds the values after each
												// capture
	 ...
	 */
	public int see(Board inputBoard, int move) {
	 ...
	 */
	private void addSlider(int startIndex, int delta, int piece) {
